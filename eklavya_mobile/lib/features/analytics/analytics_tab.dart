...
class AnalyticsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Show loading only when the primary data hasn't loaded yet
    if (summaryAsync is AsyncLoading) {
      return Scaffold(
    ...
    final weeklyXp = analytics?.dailyXp ?? List<int>.filled(7, 0);
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildLearningFocus(context, theme, goals),
                SizedBox(height: AppSpacing.lg),

                _buildCompletionRate(context, theme, completionRate, analytics),
  ...
  Widget _buildWeeklyXpCard(BuildContext context, ThemeData theme,
      List<int> weeklyXp, UserStats userStats) {
    final maxWeeklyXp =
        weeklyXp.reduce((a, b) => a > b ? a : b).toDouble();
          ...
          Row(
            ...
            children: [
              ...
              TextButton(
                ...
                child: Text('Open Roadmap',
                    style:
                        TextStyle(color: context.colors.primaryLight)),
              ),
            ],
          ),
          ...
          SizedBox(
            ...
            child: Row(
              ...
              children: List.generate(7, (i) {
                final heightRatio =
                    maxWeeklyXp > 0 ? weeklyXp[i] / maxWeeklyXp : 0.0;
                final isToday = i == 6;
  ...
  // ─── 2. Full Month Streak Calendar ──────────────────────────────────────
  Widget _buildStreakCalendar(
      BuildContext context, ThemeData theme, UserStats userStats) {
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          ...
          ...List.generate(
              ((emptySlots + daysInMonth) / 7).ceil(), (week) {
            return Padding(
  ...
  // ─── 3. Learning Focus (Domain Distribution) ────────────────────────────
  Widget _buildLearningFocus(
      BuildContext context, ThemeData theme, List<GoalItem> goals) {
    ...
    for (final goal in goals) {
      final weight = goal.milestonesCount > 0
          ...
          : 0.1;
      domainMap[goal.domain] = (domainMap[goal.domain] ?? 0) + weight + 0.1;
      ...
      child: Column(
        ...
        children: [
          ...
          if (distribution.isEmpty)
            Text(
                'No goals yet. Start a roadmap to see your focus.',
            ...
            ...distribution.entries.map((e) {
              ...
              final color =
                  _domainColors[domain] ?? context.colors.accent;
              final displayName = domain.substring(0, 1).toUpperCase() +
                  domain.substring(1);
              return Padding(
  ...
  // ─── 4. Completion Rate ──────────────────────────────────────────────────
  Widget _buildCompletionRate(BuildContext context, ThemeData theme,
      double completionRate, AnalyticsSummary? analytics) {
    return GlassCard(
      padding: EdgeInsets.all(AppSpacing.lg),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          SizedBox(height: AppSpacing.sm),
          Text(
            '${analytics?.completedTasks ?? 0} of ${analytics?.totalTasks ?? 0} tasks completed • ${analytics?.activeDaysLast30 ?? 0} active days (30d)',
